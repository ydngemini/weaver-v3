...
SCENARIOS = [
    ...
    {
        "instruction": "The n8n pipeline fails intermittently, but raw prompts cannot go to disk or be reinjected blindly. How does it recover safely?",
        "context": "n8n v6: the DLQ node returns a metadata-only failure envelope. The Cognition Mesh resilience angle tracks bounded latency/error outcomes and circuit breakers. Workflow Code nodes have no filesystem or process access.",
        "dominant": "Fracture", "qubits": [4, 0, 3], "axis": "vigilance",
        "reversal": "A safe dead-letter path is telemetry, not a second prompt store. Emit only execution ID, failed stage, bounded error class, latency, retry count, and timestamp; never persist the prompt, source evidence, model output, key, URL, or file path. Let each HTTP node perform its small bounded retry inside the 115-second workflow deadline. Then report the final outcome to the Cognition Mesh: repeated failures open that component's circuit breaker and the inference governor routes to a healthy fallback. Operators can inspect n8n's access-controlled execution record by ID, but the workflow never auto-replays private input and never claims a write that did not happen. Recovery becomes fail soft → record metadata → open circuit → route around → probe half-open, with bounded platform retention instead of an unbounded filesystem queue."
    },
    {
